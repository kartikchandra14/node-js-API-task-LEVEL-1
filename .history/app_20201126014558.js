...
app.use('/insertCandidate',(request, response) => {
    console.log('insertCandidate', request, response, request.body);
    if(!request.body){
        res.status(400);
        res.json({message: "Bad Request",Details: { error: 'request body not found !!'}});
    }else{
        let obj ={
            ...
            if(err){
                ...
                collection.insert([obj] ,function(err, res){
                    if(err){
                        console.log('error ==>',err);
                        res.status(400);
                        res.json({message: "Error",Details: { error: 'Error MESSAGE'}});
                    }else{
                        console.log('%d docs inserted', res.insertedCount);
                        res.status(200);
                        res.json({message: "Data Saved",Details: { message: 'success!!'}});
                    }
                    client.close();
                });
            }
